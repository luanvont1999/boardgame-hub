    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data && data.fcmToken) {
          ...
          debugLogs.push(`+ Tìm thấy token của UID: ${doc.id.substring(0, 6)}...`);
        }
      });
      ...
      debugLogs.push(`> [FCM] HTTP Status từ Backend: ${res.status} ${res.statusText}`);
      
      const text = await res.text();
      debugLogs.push(`> [FCM] Nội dung phản hồi thô: ${text.substring(0, 120)}${text.length > 120 ? '...' : ''}`);
    ...
    <section class="retro-console" id="status" style="margin-top: 0;">
      <div class="console-screen">
        ...
        {#if debugLogs.length > 0}
          <div style="border-top: 1.5px dashed #1e1e24; margin-top: 12px; padding-top: 10px; font-family: monospace; font-size: 0.8rem; color: #a4f0fd; text-align: left; max-height: 180px; overflow-y: auto; white-space: pre-wrap;">
